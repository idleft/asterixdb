...
public class FeedEventSubscriber implements IActiveEventSubscriber {
    ...
    public FeedEventSubscriber(FeedEventsListener listener, byte state) {
    ...
    public synchronized void notify(ActiveEvent event) {
        if (listener.getState() == state || listener.getState() == ActivityState.FAILED || listener
                .getState() == ActivityState.STOPPED) {
            done = true;
            notifyAll();
        }
    }
