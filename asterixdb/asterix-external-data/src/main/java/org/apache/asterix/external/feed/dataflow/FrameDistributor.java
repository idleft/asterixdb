...
public class FrameDistributor implements IFrameWriter {
    ...
    private final Map<EntityId, FeedFrameCollector> registeredCollectors;
    ...
    public synchronized void registerFrameCollector(FeedFrameCollector frameCollector) throws HyracksDataException {
        ...
        registeredCollectors.put(frameCollector.getCollectorId(), frameCollector);
    }

    public synchronized void deregisterFrameCollector(EntityId collectorId) throws HyracksDataException {
        if (rootFailureCause != null) {
            throw new RuntimeDataException(ErrorCode.FEED_DATAFLOW_FRAME_DISTR_REGISTER_FAILED_DATA_PROVIDER,
                    rootFailureCause);
        }
        FeedFrameCollector frameCollector = removeFrameCollector(collectorId);
        try {
            frameCollector.close();
        } catch (Throwable th) {
            rootFailureCause = th;
    ...
    public synchronized FeedFrameCollector removeFrameCollector(EntityId collectorId) {
        return registeredCollectors.remove(collectorId);
    }
