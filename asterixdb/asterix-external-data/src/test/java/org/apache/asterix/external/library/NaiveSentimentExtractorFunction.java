...
public class NaiveSentimentExtractorFunction implements IExternalScalarFunction {
    ...
    private static Properties tokenList;
    ...
    public void evaluate(IFunctionHelper functionHelper) throws Exception {
        ...
        int aggScore = 0;
        for (String token : text.getValue().replaceAll("[^A-Za-z ]+", "").split(" ")) {
            String sentiment = tokenList.getProperty(token, "0");
            aggScore += Integer.valueOf(sentiment);
        }
        String sentimentStr;
        if (aggScore == 0) {
        ...
        sentiment.setValue(sentimentStr);

        record.setField("text", text);
        record.setField("sentiment", sentiment);
